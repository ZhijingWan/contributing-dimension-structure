...
class GraphCut_CDS(SubmodularFunction):
    def __init__(self, lam: float = 1., constraint_matrix = None, **kwargs):
        super().__init__(**kwargs)
        self.lam = lam
    ...
    def calc_gain(self, idx_gain, selected, **kwargs):
        mul = self.constraint_matrix[np.ix_(selected, idx_gain)]
        gain = -2. * np.sum(self.similarity_kernel(selected, idx_gain)*mul, axis=0) + self.lam * self.sim_matrix_cols_sum[idx_gain]
